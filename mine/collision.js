...
function checkProximityToDiamondRocks() {
    ...
    const maxMiningDistance = 15; // Increased from 10 to account for larger rocks
...
function mineDiamondRock(rock) {
    ...
    if (rock.userData.health <= 0) {
        ...
        if (typeof game.logBlockGrid === 'function') {
            game.logBlockGrid();
        } else {
            console.log("Warning: game.logBlockGrid is not a function");
        }
